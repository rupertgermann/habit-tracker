...
const Dashboard=()=>{
  const navigate=useNavigate();const{habits,toggleYesNoCompletion,isLoading,hasLoaded,error}=useHabits();const{weekStartsOn}=usePreferences();const{showSuccessToast,showErrorToast}=useToast()
  const[todayHabits,setTodayHabits]=useState([]);const[weeklyData,setWeeklyData]=useState([]);const[stats,setStats]=useState({});const[showConfetti,setShowConfetti]=useState(false)
  ...
  const rate=stats.completionRate||0,completed=stats.todayCompletions||0,total=stats.totalHabits||0,streak=stats.maxStreak||0,maxWeekly=Math.max(total,1)
  const toggle=async id=>{const habit=todayHabits.find(h=>h.id===id);if(!habit)return{ok:false};const completing=!habit.isCompleted;const result=await toggleYesNoCompletion(id);if(!result.ok){showErrorToast(`Mission update failed: "${habit.name}". Try again.`);return result}const updated=todayHabits.map(h=>h.id===id?{...result.habit,isCompleted:isCompletedOnDate(result.habit,toDateKey())}:h);if(completing){showSuccessToast(`Signal confirmed: "${habit.name}".`);const count=updated.filter(h=>h.isCompleted).length;if(count===total&&total>0){setShowConfetti(true);showSuccessToast('Orbit stable. Every habit is complete.')}else if(count%3===0)setShowConfetti(true)}return result}
  ...
  if(habits.length===0)return<Shell>{header}<EmptyState type="habits" title="Launch the first habit" description="One recurring action is enough to begin a system. Add it now and give it an orbit." actionText="Add first habit" onAction={()=>navigate('/add-habit')}/></Shell>
  return<Shell initial={{opacity:0}} animate={{opacity:1}}><Confetti run={showConfetti} onComplete={()=>setShowConfetti(false)}/>{header}
    ...
    <Section aria-labelledby="missions-title"><SectionHeader><div><Kicker>01 / active trajectory</Kicker><h2 id="missions-title">Today’s missions</h2></div><Button variant="ghost" icon="plus" onClick={()=>navigate('/add-habit')}>Add Habit</Button></SectionHeader><MissionList>{todayHabits.map((habit,index)=>{const live=habits.find(h=>h.id===habit.id)||habit,active=Boolean(habit.isCompleted),count=live.type==='count';return<Mission key={habit.id} onClick={()=>navigate(`/habit/${habit.id}`)} initial={{opacity:0,x:-12}} animate={{opacity:1,x:0}} transition={{duration:.4,delay:index*.045}}>{count?<StaticStatus $active={active}>{String(index+1).padStart(2,'0')}</StaticStatus>:<MissionStatus type="button" $active={active} aria-label={`${active?'Mark as incomplete':'Mark as complete'}: ${habit.name}`} onClick={e=>{e.stopPropagation();toggle(habit.id)}}>{active?<AppIcon name="check" size={16}/>:String(index+1).padStart(2,'0')}</MissionStatus>}<MissionInfo><Glyph $color={habit.color}><AppIcon name={habit.icon} fallbackName={DEFAULT_HABIT_ICON} size={20}/></Glyph><div><h3>{habit.name}</h3><Meta>{active?'Signal active':'Awaiting signal'} · {getHabitStreak(live)}d streak</Meta></div></MissionInfo>{count&&<Control onClick={e=>e.stopPropagation()}><CountStepper habit={live}/></Control>}</Mission>})}</MissionList></Section>
