...
- `ThemeContext` resolves the selected design plus its light/dark palette, stores both preferences in the database, and removes legacy browser-storage theme values.
...
Domain tests run with `npm run test:domain` and cover habit counts, binary toggles, date-targeted changes, streak calculation, journal timeline filtering, design/theme preference resolution, and week-start normalization.

Browser tests run with `npm run test:e2e`. Playwright starts `npm run dev:e2e`, verifies the API is using `.tmp/e2e/habit-tracker.db`, and only then allows reset/restore operations. The suite covers persisted habit flows, design switching and responsive design dashboards, icon selection and legacy fallback, mobile edit stability, journal timelines, profile/avatar persistence, settings links, dark-mode controls, calendar contrast, and responsive smoke coverage.
