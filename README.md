...
| `npm run test` | Run the domain, server, and Playwright browser test suites. |
...
- `App` maps the selected design to its Dashboard, primary navigation, global styles, and responsive frame.
...
Domain tests run with `npm run test:domain` and cover Count Habit completions, Yes/No Habit toggles, date-targeted changes, streak calculation, journal timeline filtering, design/theme preference resolution, and week-start normalization.
